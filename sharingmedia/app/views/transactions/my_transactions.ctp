	...
	?>
		<div class="book_unit">	
			...
			if($transaction_collection[$i]["t"]["status"] == 1){
				echo $this->Html->link('Delete Transaction', "/transactions/delete_transaction/".$transaction_collection[$i]["b"]["title"]."/".$transaction_collection[$i]["b"]["author"]."/".$transaction_collection[$i]["u"]["name"]."/".$price."/".$loan."/".$trade."/", array(' escape' => false, 'class' => 'buttons'));
			}
			?>	
		</div>
	<?php
